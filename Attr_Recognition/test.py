...
result_dir = os.path.join('/home/minh/Documents/AI/result', model_name)
...
def get_dataloader():
    ...
    image_datasets['gallery'] = Test_Dataset(data_dir, json_path, query_gallery='gallery')
    image_datasets['query'] = Test_Dataset(data_dir, json_path, query_gallery='query')
...
# Iterate over data.
with torch.no_grad():
    for count, (images, labels, ids) in enumerate(test_loader):
        # move input to GPU
        if use_gpu:
            images = images.cuda()
        # forward
...
with open(os.path.join(result_dir, 'acc.json'), 'w') as file:
